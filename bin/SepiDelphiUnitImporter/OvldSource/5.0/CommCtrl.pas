...
type
  ...
  tagNMLVEMPTYMARKUP = packed record
    hdr: NMHDR;
...
type
  ...
  TVITEMPART = array[0..0] of Byte; const TVGIPR_BUTTON = TVITEMPART($0001); type
...
type
  ...
  PNMTVDispInfoEx = PNMTVDispInfoExA;
  { $EXTERNALSYM tagNMTVDISPINFOEXA}
  tagNMTVDISPINFOEXA = packed record
    hdr: NMHDR;
    item: TTVItemExA;
  end;
  { $EXTERNALSYM tagNMTVDISPINFOEXW}
  tagNMTVDISPINFOEXW = packed record
    hdr: NMHDR;
    item: TTVItemExW;
  end;
  {$EXTERNALSYM tagNMTVDISPINFOEX}
  tagNMTVDISPINFOEX = tagNMTVDISPINFOEXA;
...
type
  ...
  tagNMDATETIMESTRINGW = packed record
    nmhdr: TNmHdr;
    pszUserString: PWideChar;     // WideString user entered
    st: TSystemTime;           // app fills this in
    dwFlags: DWORD;            // GDT_VALID or GDT_NONE
  end;
  ...
  tagNMDATETIMEFORMATW = packed record
    ...
    pszFormat: PWideChar;                // format substring
    st: TSystemTime;                  // current systemtime
    pszDisplay: PWideChar;               // WideString to display
  ...
  tagNMDATETIMEFORMATQUERYW = packed record
    nmhdr: TNmHdr;
    pszFormat: PWideChar; // format substring
    szMax: TSize;      // max bounding rectangle app will use for this format WideString
  end;
...
function Button_SetNote(hwnd: HWND; psz: LPCWSTR): BOOL; inline;
...
function ComboBox_GetCueBannerText(hwnd: HWND; lpwText: LPCWSTR; var cchText: Integer): BOOL; inline;
