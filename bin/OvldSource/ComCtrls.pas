...
procedure TTabSheet.WMPrintClient(var Message: TWMPrintClient);
begin
  with ThemeServices do
...
procedure TTreeNode.ReadNodeData(Stream: TStream; var Info: TNodeDataInfo);
var
...
begin
  ...
  Stream.ReadBuffer(LText[1], Info.TextLen * 2);
  Text := LText;
  ImageIndex := Info.ImageIndex;
  SelectedIndex := Info.SelectedIndex;
  StateIndex := Info.StateIndex;
  ...
  if LItemCount > 0 then
    ...
    begin
      LNode := Owner.AddChild(Self, '');
      LNode.ReadNodeData(Stream, Info);
      Owner.Owner.Added(LNode);
    end;
end;

procedure TTreeNode.WriteNodeData(Stream: TStream; var Info: TNodeDataInfo);
var
...
begin
  ...
  Size := SizeOf(TNodeDataInfo) + (Info.TextLen * 2); // Length in bytes
  ...
  Stream.WriteBuffer(LText[1], Info.TextLen * 2); // Length in bytes
  for I := 0 to Info.Count - 1 do
    Item[I].WriteNodeData(Stream, Info);
end;
...
begin
  ...
  try
    ...
    if StreamVersion <> TreeNodeStreamVersion then
      Exit;
    Stream.ReadBuffer(Count, SizeOf(Count));
    for I := 0 to Count - 1 do
    begin
      LNode := Add(nil, '');
      LNode.ReadNodeData(Stream, NodeInfo);
      Owner.Added(LNode);
    end;
  finally
    FReading := False;
...
begin
  ...
  Stream.WriteBuffer(TreeNodeStreamVersion, SizeOf(Byte));
  Stream.WriteBuffer(I, SizeOf(I));
  Node := GetFirstNode;
  while Node <> nil do
  begin
    Node.WriteNodeData(Stream, NodeInfo);
    Node := Node.GetNextSibling;
  end;
end;
...
begin
  ...
  begin
    Style := Style or OrientationStyle[FOrientation] or
      ...
      TickStyles[(FMax - FMin) div Frequency > MaxAutoTicks, FTickStyle] or
...
begin
  ...
  begin
    ...
    begin
      ...
      Stream.ReadBuffer(LWideStr[1], ItemInfo.CaptionLen * 2);
      Caption := LWideStr;
      ...
      begin
        ...
        Stream.ReadBuffer(LWideStr[1], Len * 2);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Stream.WriteBuffer(LCaption[1], LCaptionLen * 2);
      end;
    end;
  end;
        ...
        if Assigned(FCanvas) then
          ...
          try
            ...
            begin
              ...
              //release the font we may have loaned during item drawing.
              if ((dwDrawStage and CDDS_ITEMPOSTPAINT) = CDDS_ITEMPOSTPAINT) and
                 (FOurFont <> 0) and (FStockFont <> 0) then
              begin
                SelectObject(hdc, FStockFont);
                DeleteObject(FOurFont);
...
begin
  ...
  if csDesigning in ComponentState then
    ...
    else if (FToolBar.Flat or ThemeServices.ThemesEnabled) and not Down then
    begin
...
begin
  ...
  if (FIndeterminate or FEnableDropdown) then Result := Result or ButtonStates[tbsIndeterminate];
  ...
  begin
    ...
    if not Enabled and (FToolbar <> nil) then
      if FEnableDropdown then
        FToolBar.Perform(TB_ENABLEBUTTON, Index, Ord(True))
      else
        FToolbar.Perform(TB_ENABLEBUTTON, Index, Ord(False));
  end;
end;
...
begin
  ...
  begin
    ...
    Perform(TB_SETEXTENDEDSTYLE, 0, CurStyle);
  end;
    ...
    begin
      ...
      begin
        if Caption <> '' then
          ...
          CaptionText := ' ';
        StrPLCopy(Buffer, CaptionText, SizeOf(Buffer));
...
var
  Button: TToolButton;
begin
  if Enabled and Showing and ShowCaptions and ContainsActiveControl then
  begin
          ...
          begin
            ...
            case nmcd.dwDrawStage of
              CDDS_ITEMPREPAINT:
                begin
                  //release the font we may have loaned during item drawing.
                  if (nmcd.dwDrawStage and CDDS_ITEMPOSTPAINT <> 0) and
                     (FOurFont <> 0) and (FStockFont <> 0) then
                  begin
...
begin
  ...
  ControlStyle := ControlStyle - [csCaptureMouse, csSetCaption] +
    [csAcceptsControls, csParentBackground];
  FButtonSize := 12;
  FDragScroll := True;
  FAutoScroll := False;
end;
...
begin
  ...
  inherited Create(AOwner);
  FComboBoxExInstance := Classes.MakeObjectInstance(ComboExWndProc); 
  ...
  if not CoInitExProcLoaded then
  begin
    CoInitExProcLoaded := True;
    Ole32Handle := Windows.GetModuleHandle('ole32.dll'); { Do not localize }
    if Ole32Handle <> 0 then
      CoInitializeExProc := Windows.GetProcAddress(Ole32Handle, 'CoInitializeEx'); { Do not localize }
  end;
  
  if Assigned(CoInitializeExProc) then
    CoInitializeExProc(nil, COINIT_MULTITHREADED or COINIT_APARTMENTTHREADED)
  else
    CoInitialize(nil);
end;
...
begin
  ...
  if HandleAllocated then
    DestroyWindowHandle;
  Classes.FreeObjectInstance(FComboBoxExInstance);
  FreeAndNil(FMemStream);
...
begin
  ...
  begin
    ...
    TPointerList(elt)[I] := CoTaskMemAlloc(Size + 2);
    FillChar(TPointerList(elt)[I]^, Size + 2, 0);
...
function TComboBoxExEnumerator.Skip(celt: Integer): HRESULT;
var
  TotalCount: Integer;
begin
  TotalCount := GetCount;
